...
public class BSONString {
	...
	public LinkedHashMap<String, Object> getBSON(String str) {
		...
		if (str.length() < 2)
			return map;
		str = str.trim();
		if (!(str.startsWith(BSON.CHAR_LEFT_BIG) && str
				.endsWith(BSON.CHAR_RIGHT_BIG)))
			return map;
	...
	private List<String> checkArraySize(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			} else if (s.equals(BSON.CHAR_LEFT_BIG)
					|| s.equals(BSON.CHAR_LEFT_MID)) {
				flag++;
			} else if (s.equals(BSON.CHAR_RIGHT_BIG)
					|| s.equals(BSON.CHAR_RIGHT_MID)) {
				flag--;
	...
	public LinkedHashMap<String, Object> getValue(String str) {
		...
		int type;
		str = str.trim();
		if (str.startsWith(BSON.CHAR_LEFT_MID)
				&& str.endsWith(BSON.CHAR_RIGHT_MID)) {
			// Object[]
			type = BSON.TYPE_ARRAY;
			List<String> array = checkArraySize(str.substring(1,
					str.length() - 1));
		...
		} else if (str.startsWith(BSON.CHAR_QUOTES)
				&& str.endsWith(BSON.CHAR_QUOTES)) {
			type = BSON.TYPE_STRING;
			map.put(BSON.TAG_VALUE, str.substring(1, str.length() - 1));
		} else if (str.startsWith(BSON.CHAR_LEFT_BIG)
				&& str.endsWith(BSON.CHAR_RIGHT_BIG)) {
		...
		} else if (str.toLowerCase(Locale.getDefault()).equals("true")
				|| str.toLowerCase(Locale.getDefault()).equals("false")) {
			type = BSON.TYPE_BOOLEAN;
			map.put(BSON.TAG_VALUE, Boolean.valueOf(str));
		} else if (str.contains(BSON.CHAR_POINT) || str.contains("e")
				|| str.contains("E")) {
			try {
	...
	private LinkedHashMap<String, Object> analyseObject(String str,
		...
		if (!tag.equals(BSON.MSG_ERROR_EMPTY)
				&& !tag.equals(BSON.MSG_ERROR_NOTBSON)) {
			map.put(BSON.TAG_TYPE, BSON.TYPE_BSON);
			BSONObject bson = new BSONObject();
			bson.put(
					tag,
					getValue((String) oMap.get(BSON.TAG_VALUE)).get(
							BSON.TAG_VALUE));
			map.put(BSON.TAG_VALUE, bson);
		} else {
