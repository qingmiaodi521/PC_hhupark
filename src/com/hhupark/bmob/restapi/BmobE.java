...
public class BmobE{
	...
	public static boolean initBmob(String appId, String restKey, int timeout){
		return Bmob.initBmob(appId, restKey, timeout);
	}
	...
	/**
	 * 文件上传
	 * @param paramContent file 完整路径文件
	 * @return String
	 */
	public static String uploadFile(String file){
		return Bmob.uploadFile(file);
	}
	 ...
	 * @return BSONObject
	 */
	public static BSONObject login(String user, String passwd){
		return resultForBSONObject(Bmob.login(user, passwd));
	}
	 ...
	 * 操作符-含有字符串
	 * @param regexValue 
	 * @return 复合含有字符串BSONObject
	 */
	public static BSONObject whereLike(String regexValue){
		return whereForBSONObject(Bmob.whereLike(regexValue));
	}
